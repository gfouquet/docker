...
# Publish any versions of the docker image not yet pushed to gfouquet/jenkins
# Arguments:
...
login-token() {
    # could use jq .token
    curl -q -sSL "https://auth.docker.io/token?service=registry.docker.io&scope=repository:gfouquet/jenkins:pull" | grep -o '"token":"[^"]*"' | cut -d':' -f 2 | xargs echo
}
...
is-published() {
    ...
    local http_code;
    http_code=$(curl $opts -q -fsL -o /dev/null -w "%{http_code}" -H "Accept: application/vnd.docker.distribution.manifest.v2+json" -H "Authorization: Bearer $TOKEN" "https://index.docker.io/v2/gfouquet/jenkins/manifests/$tag")
...
get-manifest() {
    ...
    curl $opts -q -fsSL -H "Accept: application/vnd.docker.distribution.manifest.v2+json" -H "Authorization: Bearer $TOKEN" "https://index.docker.io/v2/gfouquet/jenkins/manifests/$tag"
}
...
get-latest-versions() {
    readonly LATEST_QTY=5
    curl -q -fsSL https://repo.jenkins-ci.org/releases/org/jenkins-ci/main/jenkins-war/maven-metadata.xml | grep '<version>.*</version>' | grep -E -o '[0-9]+(\.[0-9]+)+' | sort-versions | uniq | tail -n ${LATEST_QTY}
}
...
publish() {
    ...
    docker build --file "Dockerfile$variant" \
                 ...
                 --tag "gfouquet/jenkins:${tag}" \
                 "${build_opts[@]+"${build_opts[@]}"}" .

    # " line to fix syntax highlightning
    if [ ! "$dry_run" = true ]; then
        docker push "gfouquet/jenkins:${tag}"
    fi
}
...
tag-and-push() {
    ...
    if [ "$digest_source" == "$digest_target" ] && [ -n "${digest_target}" ]; then
        ...
        docker-tag "${source}" "gfouquet" "${target}"
        if [ ! "$dry_run" = true ]; then
            echo "Pushing gfouquet/jenkins:${target}"
            docker push "gfouquet/jenkins:${target}"
        else
            echo "Would push gfouquet/jenkins:${target}"
        fi
    fi
}
